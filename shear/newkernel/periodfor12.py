...
for filenum1 in range(1,51):
	...
	#print(sim.shape)
	#calculate the max or the average value of inner product as r 
		# r= 1.0/averageinnerproduct(ussegments1,ussegments2)
		# print (r)
		r=0.0005
		...
		for i,v1 in enumerate(ussegments1):
			...
			for j,v2 in enumerate(ussegments2):
				v2=np.reshape(v2,[-1,13])
				xt= innerproduct(v1,v2)
				sim[i,j]=kernelinnerproduct(xt,r)
		np.savetxt('segmentssimilarity,r=0.0005/segmentssimilarityforEQ{0}EQ{1}'.format(str(filenum1),str(filenum2))+'.txt',sim,delimiter=',',fmt='%1.4f')
